...
def test_sink():
    ...
    sink(func=print_index, in_stream=s_array, state=0, print_list=print_list_for_array)
    ...
    sink_agent = sink_element(
        func=p, in_stream=in_stream_sink, name="sink_agent", lst=a_list
    )
    ...
    stream_to_list(
        in_stream=stm, target_list=side_effect_list_2, element_function=lambda v: 2 * v
    )
