...
def test_0_0():
    ...
    def h(proc):
        for i in range(2):
            proc.copy_stream(data=list(range(i * 3, (i + 1) * 3)), stream_name="x")
            time.sleep(0.001)
...
def multicore_example_v2(DATA, ADDEND, MULTIPLICAND, EXPONENT):
    ...
    connect_streams = [["p0", "data", "p0", "in"], ["p0", "out", "p1", "in"]]
    ...
    copy_data_to_stream(DATA, proc=process_managers["p0"], stream_name="data")
