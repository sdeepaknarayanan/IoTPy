...
def test_window_agents():
    ...
    map_window(
        func=np.mean, in_stream=r, out_stream=q, window_size=3, step_size=2, name="bb"
    )
    # ----------------------------------------------------------------
    ...
    map_window(
        func=lambda v: sum(v) + 1, in_stream=r, out_stream=u, window_size=4, step_size=4
    )
    # ----------------------------------------------------------------
    ...
    map_window(func=g, in_stream=r, out_stream=v, window_size=4, step_size=4, state=0)
    # ----------------------------------------------------------------
    ...
    merge_window(func=h, in_streams=[r, w], out_stream=x, window_size=3, step_size=3)
    merge_stream = merge_window_f(func=h, in_streams=[r, w], window_size=3, step_size=3)
    ...
    def h_with_state(list_of_windows, state):
        return (sum([sum(window) for window in list_of_windows]) + state, state + 1)
    ...
    split_window(func=splt, in_stream=r, out_streams=[y, z], window_size=3, step_size=3)
    # ----------------------------------------------------------------
    ...
    map_window(func=g, in_stream=x, out_stream=z, window_size=2, step_size=2, state=0)
    # ----------------------------------------------------------------
    ...
    def split_with_state(window, state):
        return (
            [np.sum(window, axis=0) + state, np.max(window, axis=0) + state],
            state + 1.0,
        )
    ...
    # Test window many-to-many with state and args
    def func_multi_window_with_state_and_args(windows, state, cutoff):
        max_value = np.maximum(np.max(windows[0], axis=0), np.max(windows[1], axis=0))
        max_value = np.maximum(max_value, cutoff) + state

        min_value = np.minimum(np.min(windows[0], axis=0), np.min(windows[1], axis=0))
    ...
    x.extend(np.array([[1.0, 5.0], [7.0, 11.0]]))
    a.extend(np.array([[0.0, 1.0], [2.0, 3.0]]))
    scheduler.step()
    # sum_array_list is the sum of x with window size, step size of 2
    assert np.array_equal(sum_array_list, [np.array([1.0 + 7.0, 5.0 + 11.0])])
    # y is the mean of x with window size and step size of 2
    assert np.array_equal(recent_values(x), np.array([[1.0, 5.0], [7.0, 11.0]]))
    assert np.array_equal(
        recent_values(y), np.array([[(1.0 + 7.0) // 2.0, (5.0 + 11.0) // 2.0]])
    )
    assert np.array_equal(
        recent_values(t), np.array([[(1.0 + 5.0) // 2.0, (7.0 + 11.0) // 2.0]])
    )
    assert np.array_equal(recent_values(z), np.array([[1.0 + 7.0, 5.0 + 11]]))
    assert np.array_equal(
        recent_values(b), [np.array([1.0 + 7.0 + 0.0 + 2.0, 5.0 + 11.0 + 1.0 + 3.0])]
    )
    assert np.array_equal(recent_values(c), np.array([[8.0, 16.0]]))
    assert np.array_equal(recent_values(d), np.array([[7.0, 11.0]]))
    assert np.array_equal(recent_values(r), np.array([[10.0, 11.0]]))
    assert np.array_equal(recent_values(s), np.array([[0.0, 1.0]]))

    a.extend(np.array([[0.0, 1.0], [1.0, 0.0]]))
    scheduler.step()
    assert np.array_equal(
        recent_values(y), np.array([[(1.0 + 7.0) / 2.0, (5.0 + 11.0) / 2.0]])
    )
    ...
    x.extend(np.array([[14.0, 18.0], [18.0, 30.0], [30.0, 38.0], [34.0, 42.0]]))
    scheduler.step()
    assert np.array_equal(
        recent_values(y), np.array([[4.0, 8.0], [16.0, 24.0], [32.0, 40.0]])
    )
    assert np.array_equal(
        recent_values(z), np.array([[8.0, 16.0], [33.0, 49.0], [66.0, 82.0]])
    )
    assert np.array_equal(
        recent_values(c), np.array([[8.0, 16.0], [33.0, 49.0], [66.0, 82.0]])
    )
    assert np.array_equal(
        recent_values(d), np.array([[7.0, 11.0], [19.0, 31.0], [36.0, 44.0]])
    )
    assert np.array_equal(recent_values(r), np.array([[10.0, 11.0], [19.0, 31.0]]))
    assert np.array_equal(recent_values(s), np.array([[0.0, 1.0], [1.0, 1.0]]))
