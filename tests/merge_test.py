...
class merge_test(unittest.TestCase):
    def test_some_merge_agents(self):
        ...
        ww_stream, xx_stream = multi_element_f(
            func=f_many, in_streams=[u_stream, v_stream], num_out_streams=2
        )
        ...
        scheduler.step()
        assert recent_values(w_args_kwargs_stream) == [0, 10, 20, 30, 40, 240]
        assert recent_values(x_args_kwargs_stream) == [10, 15, 20, 25, 30, 130]
        assert recent_values(ww_args_kwargs_stream) == recent_values(
            w_args_kwargs_stream
        )
        assert recent_values(xx_args_kwargs_stream) == recent_values(
        ...
        scheduler.step()
        assert recent_values(w_args_kwargs_stream) == [0, 10, 20, 30, 40, 240, 448, 656]
        ...
        expected_array = np.sum(
            [np.linspace(0.0, 4.0, 5), np.linspace(0.0, 4.0, 5)], axis=0
        )
        ...
        scheduler.step()
        assert np.array_equal(recent_values(z), [(0, 10.0), (1, 1.0)])

        y.append(np.array(2.0))
        scheduler.step()
        assert np.array_equal(recent_values(z), [(0, 10.0), (1, 1.0), (1, 2.0)])

        x.append(np.array(20.0))
        scheduler.step()
        assert np.array_equal(
            recent_values(z), [(0, 10.0), (1, 1.0), (1, 2.0), (0, 20.0)]
        )
        ...
        merge_stream = merge_window_f(
            func=h, in_streams=[r, w], window_size=3, step_size=3
        )
    ...
    def test_timed_mix_agents(self):
        ...
        y.append((2, "A"))
        scheduler.step()
        assert recent_values(z) == [(0, (0, "a")), (1, (0, "b")), (2, (1, "A"))]
    ...
    def test_operator_overloading(selfself):
        ...
        Stream.scheduler.step()
        assert recent_values(z) == [100, 103, 106, 109, 112, 115, 118, 121, 124, 127]
        assert recent_values(a) == [100, 101, 102, 103, 104, 105, 106, 107, 108, 109]
        assert recent_values(b) == [0, 102, 208, 318, 432, 550, 672, 798, 928, 1062]
