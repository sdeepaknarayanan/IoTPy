...
def test_split_agents():
    ...
    r = Stream("r")
    t = Stream("t")
    e = split_element(func=h, in_stream=in_stream_split, out_streams=[r, t], name="e")
    r_split, t_split = split_element_f(
        function=h, in_stream=in_stream_split, num_out_streams=2,
    )
    r_args, t_args = split_element_f(h_args, in_stream_split, 2, addend=1, multiplier=2)
    ...
    d_separate = separate(
        in_stream=s_separate, out_streams=[x_separate, u_separate], name="d separate"
    )
    ...
    split_window(func=f, in_stream=x, out_streams=[y, z], window_size=2, step_size=2)
    ...
    split_window(func=f, in_stream=x, out_streams=[y, z], window_size=3, step_size=3)
