...
def test_multithread_1_1():
    ...
    iot_thread_1 = threading.Thread(
        target=thread_target_extending, args=(q_in_1, [q_out_1], [x, y], finished)
    )

    # IoTPy thread_2
    iot_thread_2 = threading.Thread(
        target=thread_target_extending, args=(q_in_2, [q_out_2], [u], finished)
    )

    # Threads to read output queues of IoTPy threads.
    output_1 = []
    output_thread_1 = threading.Thread(
        target=output_thread_target, args=(q_out_1, output_1, finished)
    )

    output_2 = []
    output_thread_2 = threading.Thread(
        target=output_thread_target, args=(q_out_2, output_2, finished)
    )
...
def test_multithread_1():
    ...
    iot_thread = threading.Thread(
        target=thread_target_appending, args=(q_in, [q_out], in_streams)
    )

    output_thread = threading.Thread(
        target=output_thread_target, args=(q_out, output, finished)
    )
