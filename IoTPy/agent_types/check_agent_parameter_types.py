...
def check_list_of_streams_type(list_of_streams, agent_name, parameter_name):
    ...
    """
    assert (
        list_of_streams is None
        or isinstance(list_of_streams, list)
        or isinstance(
            list_of_streams,
            tuple,
            "Call to create agent named, {0}, has parameter, {1},"
            " with an argument, {2}, which is not a list or tuple".format(
                agent_name, parameter_name, list_of_streams
            ),
        )
    )
    ...
    for stream in list_of_streams:
        assert isinstance(stream, Stream) or isinstance(stream, StreamArray), (
            "Call to create agent named, {0}, has parameter, {1},"
            " with an argument, {2}, which is not a Stream".format(
                agent_name, parameter_name, stream
            )
        )
...
def check_function_type(name, func):
    assert callable(
        func
    ), "Call to create agent named, {0}, has a parameter func, {1}, which is not a function".format(
        name, func
    )
...
def check_stream_type(name, in_or_out_stream_str, stream):
    assert isinstance(stream, Stream) or isinstance(stream, StreamArray), (
        ...
        " is not a single stream.".format(name, in_or_out_stream_str, stream)
    )


def check_in_lists_type(name, in_lists, num_in_streams):
    assert isinstance(in_lists, list) or isinstance(
        in_lists, tuple
    ), "Error in transition of agent named {0}. in_lists {1} is not a list.".format(
        name, in_lists
    )
    assert len(in_lists) == num_in_streams, (
        "Error in transition of agent named {0}. The length of in_lists {1} is"
        " equal to the number {2} of input streams".format(
            name, in_lists, num_in_streams
        )
    )
...
def check_num_args_in_func_with_state(name, func, func_args, func_kwargs):
    assert isinstance(func, types.FunctionType) or isinstance(
    ...
    assert len(args) <= 2 + len(func_args) + len(func_kwargs), (
        ...
        "and keyword args for func are {4}".format(
            name, func.__name__, args, func_args, func_kwargs
        )
    )
...
def check_func_output_for_multiple_streams(
    ...
    assert all(
        [
            isinstance(snapshot, list) or isinstance(snapshot, tuple)
        ...
        " or a tuple. Here function, {2}, returns {3}".format(
            name, func.__name__, func.__name__, output_snapshots
        )
    )
    assert all([len(snapshot) == num_out_streams for snapshot in output_snapshots]), (
        ...
        " Function, {3}, returned {4}".format(
            name, func.__name__, num_out_streams, func.__name__, output_snapshots
        )
    )
...
def check_split_agent_arguments(func, in_stream, out_streams, call_streams, name):
    ...
    check_stream_type(name, "in_stream", in_stream)
    check_list_of_streams_type(
        list_of_streams=out_streams, agent_name=name, parameter_name="out_streams"
    )
    check_list_of_streams_type(
        list_of_streams=call_streams, agent_name=name, parameter_name="call_streams"
    )
...
def check_window_and_step_sizes(name, window_size, step_size):
    ...
    assert isinstance(
        step_size, int
    ), "agent {0} created with step_size {1} that is not an int".format(name, step_size)
    assert (
        window_size > 0
...
def check_source_function_arguments(
    func, out_stream_name, time_interval, num_steps, window_size, state, name
):
    check_function_type(name, func)
    assert time_interval >= 0
    assert num_steps is None or isinstance(
        num_steps, int
...
def check_source_file_arguments(
    ...
    check_function_type(name, func)
    assert isinstance(filename, str), "filename should be a string but is {0}: ".format(
        filename
    )
    assert (
        time_interval >= 0
