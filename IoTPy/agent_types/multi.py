    ...
    def transition(in_lists, state):
        ...
        if state is None:
            ...
            for i in range(len(input_snapshots)):
                output_snapshots[i], state = func(
                    input_snapshots[i], state, *args, **kwargs
                )
        ...
        add_to_output_streams = [
            list(snapshot) for snapshot in list(zip(*output_snapshots))
        ]
...
def multi_element_f(func, in_streams, num_out_streams, state=None, *args, **kwargs):
    out_streams = [Stream() for _ in range(num_out_streams)]
    call_streams = None
    name = None
    multi_element(
        func, in_streams, out_streams, state, call_streams, name, *args, **kwargs
    )
    return out_streams
...
def multi_list_f(func, in_streams, num_out_streams, state=None, *args, **kwargs):
    ...
    multi_list(func, in_streams, out_streams, state, None, None, *args, **kwargs)
    return out_streams
...
def multi_window(
    ...
    **kwargs
    ...
    def transition(in_lists, state=None):
        ...
        output_lists = [
            output_snapshot for output_snapshot in list(zip(*output_snapshots))
        ]
