...
# -----------------------------------------------------------------------
def sink_element(
    func, in_stream, state=None, call_streams=None, name=None, *args, **kwargs
):
    """
...
# -----------------------------------------------------------------------
def signal_sink(func, in_stream, state=None, name=None, *args, **kwargs):
    """
...
# -----------------------------
def stream_to_list(in_stream, target_list, element_function=None, state=None, **kwargs):
    def simple_append(element, target_list):
        target_list.append(element)

    def function_stateless_append(element, target_list, element_function, **kw):
        target_list.append(element_function(element, **kw))

    def function_stateful_append(element, state, target_list, element_function, **kw):
...
# -----------------------------
def stream_to_file(in_stream, filename, element_function=None, state=None, **kwargs):
    def simple_append(element, filename):
        with open(filename, "a") as the_file:
            the_file.write(str(element) + "\n")

    def function_stateless_append(element, filename, element_function, **kw):
        with open(filename, "a") as the_file:
            the_file.write(str(element_function(element, **kw)) + "\n")

    def function_stateful_append(element, state, filename, element_function, **kw):
        next_output, next_state = element_function(element, state, **kw)
        with open(filename, "a") as the_file:
...
# ----------------------------------------------------
def stream_to_queue(in_stream, queue, element_function=None, state=None, **kwargs):
    def simple_append(element, queue):
        queue.put(element)

    def function_stateless_append(element, queue, element_function, **kw):
        queue.put(element_function(element, **kw))

    def function_stateful_append(element, state, queue, element_function, **kw):
        next_output, next_state = element_function(element, state, **kw)
...
def sink_list_f(func, in_stream, state=None, *args, **kwargs):
    ...
    sink_list(func, in_stream, state, None, None, *args, **kwargs)
    return out_stream
