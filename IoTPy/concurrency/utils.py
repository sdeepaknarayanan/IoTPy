    ...
    for process_name in processes:
        ...
        if "sources" in process.keys():
            ...
            for stream in process["sources"]:
                stream_name_type_list.append(
                    (stream, process["sources"][stream]["type"])
                )
        for stream_name_type in stream_name_type_list:
            # check stream_name_type format validity
            assert (
                type(stream_name_type) == tuple
            ), "stream_name_type of process '{}' is not a tuple".format(process_name)
            # check stream name validity
            assert (
                type(stream_name_type[0]) == str
            ), "stream '{}' of process '{}' has an invalid name".format(
                stream_name_type[0], process_name
            )
            # check stream type validity
            assert (
                stream_name_type[1] in type_code_set
            ), "stream '{}' of process '{}' does not have a valid data type".format(
                stream_name_type[0], process_name
            )

            # assert that there is not duplicate stream name in process
            assert (
                stream_name_type[0] not in stream_name_set
            ), "process '{}' contains duplicate stream name '{}'".format(
                process_name, stream_name_type[0]
            )
    ...
    for process_name in connections:
        for stream in connections[process_name]:
            for target_process_stream in connections[process_name][stream]:
                assert (
                    type(target_process_stream) == tuple
                    or type(target_process_stream) == list
                ), "connection for stream '{}' of process '{}' is '{}' which is not a tuple or list".format(
                    stream, process_name, target_process_stream
                )
                assert (
                    len(target_process_stream) == 2
                ), "connection for stream '{}' of process '{}' is ill-formatted".format(
                    stream, process_name
                )
                assert (
                    type(target_process_stream[0]) == str
                    and type(target_process_stream[1]) == str
                ), "connection for stream '{}' of process '{}' does not have str elements".format(
                    stream, process_name
                )
...
def check_connections_validity(processes, connections, external_sources={}):
    ...
    for process in processes:
        process_stream_dict[process] = {
            "in_stream": [],
            "out_stream": [],
            "source": [],
        }  # list of stream names
        process_stream_type_dict[
            process
        ] = {}  # dictionary to track data types of each stream
        ...
        if "sources" in processes[process].keys():
            for source in processes[process]["sources"]:
                ...
                process_stream_type_dict[process][source] = processes[process][
                    "sources"
                ][source]["type"]
    ...
    for process in connections:
        for stream in connections[process]:
            ...
            if not stream in external_sources:
                assert (
                    stream in process_stream_dict[process]["out_stream"]
                    or stream in process_stream_dict[process]["source"]
                ), "stream '{}' from process '{}' is not defined in processes".format(
                    stream, process
                )
            ...
            for target_process, target_stream in connections[process][stream]:
                ...
                assert (
                    target_process in process_stream_dict
                ), "process '{}' connected to '{}' does not exist".format(
                    target_process, process
                )
                # assert that the input stream for the target process is actually an input stream of the process
                assert (
                    target_stream in process_stream_dict[target_process]["in_stream"]
                ), "process '{}' does not have in_stream '{}'".format(
                    target_process, target_stream
                )
        ...
        for source_name in process_stream_dict[process]["source"]:
            ...
            assert (
                source_cnt > 0
            ), "source '{}' must be connected to at least one input stream of any process".format(
                source_name
            )
            ...
            for process_name in connections:
                if source_name in connections[process_name]:
                    for target_process, target_stream in connections[process_name][
                        source_name
                    ]:
                        assert (
                            target_process in process_stream_dict
                        ), "process '{}' connected to source '{}' does not exist".format(
                            target_process, source_name
                        )
                        assert (
                            target_stream
                            in process_stream_dict[target_process]["in_stream"]
                        ), "source '{}' is not connected to stream '{}'".format(
                            source_name, target_stream
                        )
                ...
                for stream_name in connections[process_name]:
                    for target_process, target_stream in connections[process_name][
                        ...
                        if (
                            ...
                            and in_stream_name == target_stream
                        ):
                            in_stream_cnt += 1
            assert (
                in_stream_cnt == 1
            ), "in_stream '{}' of process '{}' is connected to {} streams".format(
                in_stream_name, process, in_stream_cnt
            )
    ...
    for process in connections:
        if process != "external_sources":
            for stream in connections[process]:
                for target_process, target_stream in connections[process][stream]:
                    if stream in (
                        process_stream_dict[process]["out_stream"]
                        + process_stream_dict[process]["source"]
                    ):
                        assert (
                            process_stream_type_dict[process][stream]
