...
def delete(v, deletion_probability=0.1, duplication_probability=0.1):
    ...
    if random_number < deletion_probability:
        return _no_value
    # Duplicate message
    elif random_number < deletion_probability + duplication_probability:
...
def proposer_behavior(input_msg, state):
    ...
    logging.info(
        "time remaining to next prepare message is " + str(time_to_next_prepare_msg)
    )
    ...
    if msg_name == "time_step":
        ...
        if time_to_next_prepare_msg == 0:
            ...
            next_state = (
                my_id,
                my_prepare_timestamp,
                time_to_next_prepare_msg,
                dict_of_acceptors,
            )
        ...
        if (prepare_timestamp, prepare_proposer) == (my_prepare_timestamp, my_id):
            ...
            if len(dict_of_acceptors) >= majority:
                ...
                max_accept_id = max(
                    acceptor_values[1]
                    for acceptor_values in dict_of_acceptors.values()
...
def acceptor_behavior(input_msg, state):
    ...
    if (
        msg_timestamp > my_prepare_timestamp
        or (msg_timestamp == my_prepare_timestamp)
        and (msg_proposer >= prepare_proposer)
    ):
...
def run_Paxos():
    ...
    time_step_agent = blend(
        func=time_step_behavior,
