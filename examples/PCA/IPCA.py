    ...
    def compute_func(in_streams, out_streams):
        def partial_fit(X, state):
            ...
            col_mean, col_var, n_total_samples = _incremental_mean_and_var(
                X,
                last_mean=state.mean_,
                last_variance=state.var_,
            ...
            if state.n_samples_seen_ == 0:
                ...
                mean_correction = np.sqrt(
                    (state.n_samples_seen_ * n_samples) / n_total_samples
                ) * (state.mean_ - col_batch_mean)
                X = np.vstack(
                    (
                        state.singular_values_.reshape((-1, 1)) * state.components_,
                        X,
                        mean_correction,
                    )
                )
            ...
            state.explained_variance_ratio_ = explained_variance_ratio[
                : state.n_components_
            ]
            if state.n_components_ < n_features:
                state.noise_variance_ = explained_variance[state.n_components_ :].mean()
            else:
                state.noise_variance_ = 0.0
