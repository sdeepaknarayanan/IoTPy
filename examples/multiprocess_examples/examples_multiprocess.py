...
@multi_e
def max_min(any_list):
    return max(any_list), min(any_list)
...
def example_1():
    ...
    mp = Multiprocess(
        processes=[proc_0, proc_1], connections=[(proc_0, "out", proc_1, "in")]
    )
    mp.run()
...
def example_4():
    ...
    def source(s):
        return source_func_to_stream(func=random.random, out_stream=s, num_steps=10)
...
def example_5():
    ...
    for i in range(num_procs):
        proc = shared_memory_process(
            ...
            out_stream_names,
            connect_sources=[
                (in_stream_name, source) for in_stream_name in in_stream_names
            ],
            name="process" + str(i),
        )
        procs.append(proc)
    ...
    vm = Multiprocess(
        ...
        connections=[(proc, "max_values", aggregator, "max_val") for proc in procs]
