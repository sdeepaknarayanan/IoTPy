...
def f(in_streams, out_streams):
    ...
    # Wrap the terminating function to create an agent
    zip_map(func=magnitude_of_vector, in_streams=zero_means, out_stream=magnitudes)
    ...
    # Wrap the terminating function to create an agent
    map_element(func=simple_anomaly, in_stream=magnitudes, out_stream=out_streams[0])
...
if __name__ == "__main__":
    ...
    sensors = {
        "S1": {"e": "S1.e.txt", "n": "S1.n.txt", "z": "S1.z.txt"},
        "S2": {"e": "S2.e.txt", "n": "S2.n.txt", "z": "S2.z.txt"},
        "S3": {"e": "S3.e.txt", "n": "S3.n.txt", "z": "S3.z.txt"},
    }
    ...
    for sensor_name in sensors.keys():
        ...
        for direction in directions:
            ...
            def source_thread_target(
                proc, stream_name, filename=sensors[sensor_name][direction]
            ):
    ...
    for sensor_name in sensors.keys():
        processes[sensor_name + "_process"] = {
            "in_stream_names_types": [
                ("in_" + direction, "f") for direction in directions
            ],
    ...
    processes["aggregation_process"] = {
        "in_stream_names_types": [
            ("in_" + sensor_name, "f") for sensor_name in sensors.keys()
        ],
    ...
    for sensor_name in sensors.keys():
        ...
        for direction in directions:
            connections[process_name]["source_" + direction] = [
                (process_name, "in_" + direction)
            ]
