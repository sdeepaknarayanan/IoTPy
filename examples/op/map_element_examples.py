...
map_element(func=multiply_and_add, in_stream=x, out_stream=y, multiplicand=2, addend=10)
...
map_element(func=add_twice_position, in_stream=x, out_stream=y, state=0)
# y[j] = x[j] + 2*j
...
map_element(func=add_twice_position, in_stream=x, out_stream=y, state=10)
# y[j] = x[j] + 2*j + 10
...
def f(in_stream_element, current_state, multiplicand, addend):
    out_stream_element = in_stream_element * multiplicand + current_state
    next_state = current_state + addend
    return out_stream_element, next_state


map_element(func=f, in_stream=x, out_stream=y, state=0, multiplicand=3, addend=2)
# Initial state is 0.
