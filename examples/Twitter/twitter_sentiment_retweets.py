...
def get_text(tweet):
                ...
                if "extended_tweet" in str(tweet["retweeted_status"]):
                    if "extended_tweet" in tweet["retweeted_status"].keys():
                        ...
                        if "quoted_status" in (tweet["retweeted_status"]).keys():
                            if (
                                "extended_tweet"
                                in (tweet["retweeted_status"]["quoted_status"]).keys()
                            ):
                                text = tweet["retweeted_status"]["quoted_status"][
                                    "extended_tweet"
                                ]["full_text"]
    else:
        try:
    ...
    # clean text
    text = text.replace("&amp", "and").replace("\n", " ").replace("RT ", "")
...
def print_sentiment_followers_retweets(tweet):
    ...
    if followers >= follower_limit and retweets >= retweet_limit:
        ...
        print("Followers: " + str(followers) + ", Retweets: " + str(retweets) + "\n")
