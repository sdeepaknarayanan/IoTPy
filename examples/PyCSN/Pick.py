...
def pick_orientation(scaled, timestamps, orientation):
    ...
    map_window(
        lambda window: window[-1] - sum(window) / float(len(window)),
        decimated,
        demeaned,
    ...
    zip_streams(
        in_streams=[decimated_timestamps, demeaned], out_stream=timestamped_data
    )
...
def picker(nezt):
    ...
    split_element(
        lambda nezt: [nezt[0], nezt[1], nezt[2], nezt[3]],
        in_stream=nezt,
        out_streams=[n, e, z, t],
    )
