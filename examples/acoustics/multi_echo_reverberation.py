...
class make_aggregator(object):
    ...
    def __init__(self, original_sound_list, output_file_name, echo_names):
...
class make_echo(object):
    ...
    def echo_func(self, in_streams, out_streams):
        ...
        # CREATE INTERNAL STREAMS.
        original_sound_plus_echo = Stream(name="original sound plus echo")
        ...
        stream_to_file(in_stream=echo, filename=self.echo_name + ".txt")
        stream_to_file(
            in_stream=original_sound_plus_echo,
...
def two_echo_reverberation(
    original_sound_list, delays_and_attenuations_dict, output_file_name
):
    """
    Parameters
    ----------
    ...
    aggregator = make_aggregator(original_sound_list, output_file_name, echo_names)
    ...
    for echo_name, delay_and_attenuation in delays_and_attenuations_dict.items():
        ...
        echo = make_echo(delay, attenuation_vector, echo_name)
        ...
        connections.append(
            (aggregator_process, "original_sound_copy", echo_process, "original_sound")
        )
        # Create a connection from the echo process' output,
        # called echo_name, to the aggregator's input also
        # called echo_name.
        connections.append((echo_process, echo_name, aggregator_process, echo_name))
...
def test_reverberation():
    ...
    delays_and_attenuations_dict = {"echo_1": (2, [0.8]), "echo_2": (3, [0.4])}
