...
def simple_reverberation(original_sound_list, attenuation_vector, delay):
    ...
    def generate_original_sound(original_sound):
        return source_list_to_stream(
            in_list=original_sound_list, out_stream=original_sound, time_interval=0
        )
    ...
    def compute_func(in_streams, out_streams):
        ...
        zip_map(func=sum, in_streams=[original_sound, echo], out_stream=heard_sound)
        # Agent that creates the echo from the heard sound.
        window_dot_product(
            in_stream=heard_sound,
            out_stream=echo,
